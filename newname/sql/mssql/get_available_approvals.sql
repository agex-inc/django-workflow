WITH approvals_with_min_priority (workflowmodel_id, transition_id, object_id, min_priority) AS
         (
             ...
             FROM newname.dbo.newname_transitionapproval
     ...
     authorized_approvals(id, workflowmodel_id, transition_id, source_state_id, object_id, priority) AS
         (
             SELECT ta.id,
                    ...
                    ta.object_id,
                    ta.priority
             FROM newname.dbo.newname_transitionapproval ta
                      INNER JOIN newname.dbo.newname_transition t on t.id = ta.transition_id
                      LEFT JOIN newname.dbo.newname_transitionapproval_permissions tap on tap.transitionapproval_id = ta.id
                      LEFT JOIN newname.dbo.newname_transitionapproval_groups tag on tag.transitionapproval_id = ta.id
