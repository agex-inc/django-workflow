...
from newname.driver.newname_driver import NewNamenewname
from newname.models import TransitionApproval


class MsSqlnewname(NewNamenewname):

    def __init__(self, *args, **kwargs):
        super(MsSqlnewname, self).__init__(*args, **kwargs)
        with open(os.path.join(dirname(dirname(__file__)), "sql", "mssql", "get_available_approvals.sql")) as f:
            self.available_approvals_sql_template = f.read().replace("newname.dbo.", "")
