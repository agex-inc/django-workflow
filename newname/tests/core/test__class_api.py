...
from newname.models.factories import PermissionObjectFactory, UserObjectFactory, StateObjectFactory, GroupObjectFactory
from newname.tests.models import BasicTestModel
# noinspection PyMethodMayBeStatic,DuplicatedCode
from newnametest.flowbuilder import RawState, AuthorizationPolicyBuilder, FlowBuilder
...
class ClassApiTest(TestCase):
    ...
    def test_shouldReturnNoApprovalWhenUserIsUnAuthorized(self):
        ...
        available_approvals = BasicTestModel.newname.my_field.get_available_approvals(as_user=unauthorized_user)
    ...
    def test_shouldReturnAnApprovalWhenUserIsAuthorizedWithAPermission(self):
        ...
        available_approvals = BasicTestModel.newname.my_field.get_available_approvals(as_user=authorized_user)
        assert_that(available_approvals, has_length(1))
        assert_that(list(available_approvals), has_item(
            all_of(
                has_property("workflowmodel_object", workflowmodel_object),
                has_property("workflowmodel", flow.workflowmodel),
    ...
    def test__shouldReturnApprovalsOnTimeWhenTooManyWorkflowModelObject(self):
        authorized_permission = PermissionObjectFactory()
        authorized_user = UserObjectFactory(user_permissions=[authorized_permission])
        ...
        before = datetime.now()
        approvals = BasicTestModel.newname.my_field.get_on_approval_objects(as_user=authorized_user)
        after = datetime.now()
    ...
    def test_shouldAssesInitialStateProperly(self):
        ...
        assert_that(BasicTestModel.newname.my_field.initial_state, equal_to(flow.get_state(state1)))
    ...
    def test_shouldAssesFinalStateProperlyWhenThereIsOnlyOne(self):
        ...
        assert_that(BasicTestModel.newname.my_field.final_states, has_length(1))
        assert_that(list(BasicTestModel.newname.my_field.final_states), has_item(flow.get_state(state2)))
    ...
    def test_shouldAssesFinalStateProperlyWhenThereAreMultiple(self):
        ...
        assert_that(BasicTestModel.newname.my_field.final_states, has_length(4))
        assert_that(
            list(BasicTestModel.newname.my_field.final_states),
            has_items(
