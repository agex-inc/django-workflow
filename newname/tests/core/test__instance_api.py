...
from newname.models import TransitionApproval, PENDING, CANCELLED, APPROVED, Transition, JUMPED
from newname.models.factories import UserObjectFactory, PermissionObjectFactory
from newname.tests.matchers import has_approval
from newname.tests.models import BasicTestModel, ModelWithTwoStateFields, ModelWithStringPrimaryKey
from newname.tests.models.factories import ModelWithTwoStateFieldsObjectFactory
from newname.utils.exceptions import NewNameException
# noinspection PyMethodMayBeStatic,DuplicatedCode
from newnametest.flowbuilder import FlowBuilder, AuthorizationPolicyBuilder, RawState
...
class InstanceApiTest(TestCase):
    ...
    def test_shouldNotReturnOtherObjectsApprovalsForTheAuthorizedUser(self):
        ...
        available_approvals = workflowmodel_object1.newname.my_field.get_available_approvals(as_user=authorized_user)
    ...
    def test_shouldNotAllowUnauthorizedUserToProceedToNextState(self):
        ...
        assert_that(
            calling(workflowmodel_object.newname.my_field.approve).with_args(as_user=unauthorized_user),
            raises(NewNameException, "There is no available approval for the user")
        )
    ...
    def test_shouldAllowAuthorizedUserToProceedToNextState(self):
        ...
        workflowmodel_object.newname.my_field.approve(as_user=authorized_user)
    ...
    def test_shouldNotLetUserWhosePriorityComesLaterApproveProceed(self):
        ...
        assert_that(
            calling(workflowmodel_object.newname.my_field.approve).with_args(as_user=manager),
            raises(NewNameException, "There is no available approval for the user")
        )
    ...
    def test_shouldNotTransitToNextStateWhenThereAreMultipleApprovalsToBeApproved(self):
        ...
        assert_that(workflowmodel_object.my_field, equal_to(flow.get_state(state1)))
        workflowmodel_object.newname.my_field.approve(team_leader)
        assert_that(workflowmodel_object.my_field, equal_to(flow.get_state(state1)))
    ...
    def test_shouldTransitToNextStateWhenAllTheApprovalsAreApproved(self):
        ...
        assert_that(workflowmodel_object.my_field, equal_to(flow.get_state(state1)))
        workflowmodel_object.newname.my_field.approve(manager)
        assert_that(workflowmodel_object.my_field, equal_to(flow.get_state(state2)))
    ...
    def test_shouldDictatePassingNextStateWhenThereAreMultiple(self):
        ...
        workflowmodel_object = flow.objects[0]
        assert_that(
            calling(workflowmodel_object.newname.my_field.approve).with_args(as_user=authorized_user),
            raises(NewNameException, "State must be given when there are multiple states for destination")
        )
    ...
    def test_shouldTransitToTheGivenNextStateWhenThereAreMultipleNextStates(self):
        ...
        assert_that(workflowmodel_object.my_field, equal_to(flow.get_state(state1)))
        workflowmodel_object.newname.my_field.approve(as_user=authorized_user, next_state=flow.get_state(state3))
    ...
    def test_shouldNotAcceptANextStateWhichIsNotAmongPossibleNextStates(self):
        ...
        assert_that(
            calling(workflowmodel_object.newname.my_field.approve).with_args(as_user=authorized_user, next_state=flow.get_state(invalid_state)),
            raises(NewNameException,
    ...
    def test_shouldAllowCyclicTransitions(self):
        ...
        workflowmodel_object.newname.my_field.approve(as_user=authorized_user, next_state=flow.get_state(cycle_state_1))
        assert_that(workflowmodel_object.my_field, equal_to(flow.get_state(cycle_state_1)))
    ...
    def test__shouldReturnNextApprovals(self):
        ...
        assert_that(workflowmodel_object.my_field, equal_to(flow.get_state(state1)))
        next_approvals = workflowmodel_object.newname.my_field.next_approvals
    ...
    def test_shouldNotCancelDescendantsIfItIsPartOfPossibleFuture(self):
        ...
        workflowmodel_object.newname.my_field.approve(as_user=authorized_user, next_state=flow.get_state(diamond_left_state_1))
    ...
    def test_shouldJumpToASpecificState(self):
        ...
        workflowmodel_object.newname.my_field.jump_to(flow.get_state(state3))
    ...
    def test_shouldNotJumpBackToAPreviousState(self):
        ...
        workflowmodel_object.newname.my_field.approve(as_user=authorized_user, next_state=flow.get_state(state2))
        assert_that(
            calling(workflowmodel_object.newname.my_field.jump_to).with_args(flow.get_state(state1)),
            raises(NewNameException, "This state is not available to be jumped in the future of this object")
        )
    ...
    def test_shouldNotCrashWhenAModelObjectWithStringPrimaryKeyIsApproved(self):
        ...
        workflowmodel_object.newname.status.approve(as_user=authorized_user)
    ...
    def test_shouldAllowMultipleCyclicTransitions(self):
        ...
        workflowmodel_object.newname.my_field.approve(as_user=authorized_user, next_state=flow.get_state(cycle_state_2))
        assert_that(workflowmodel_object.my_field, equal_to(flow.get_state(cycle_state_2)))
