...
from newname.models import TransitionApproval, OnApprovedHook, Function, OnCompleteHook, OnTransitHook
from newname.models.factories import StateObjectFactory, WorkflowModelFactory, TransitionApprovalMetaFactory, PermissionObjectFactory, UserObjectFactory, \
...
def clean_migrations():
    for f in os.listdir("newname/tests/volatile/newname/"):
        if f != "__init__.py" and f != "__pycache__":
            os.remove(os.path.join("newname/tests/volatile/newname/", f))

    for f in os.listdir("newname/tests/volatile/newname_tests/"):
        if f != "__init__.py" and f != "__pycache__":
            os.remove(os.path.join("newname/tests/volatile/newname_tests/", f))


class MigrationTests(TestCase):
    """
    This is the case to detect missing migration issues like https://github.com/javrasya/django-newname/issues/30
    """
    ...
    @override_settings(MIGRATION_MODULES={"newname": "newname.tests.volatile.newname"})
    def test_shouldCreateAllMigrations(self):
        for f in os.listdir("newname/migrations"):
            if f != "__init__.py" and f != "__pycache__" and not f.endswith(".pyc"):
                open(os.path.join("newname/tests/volatile/newname/", f), 'wb').write(open(os.path.join("newname/migrations", f), 'rb').read())

        self.migrations_before = list(filter(lambda f: f.endswith('.py') and f != '__init__.py', os.listdir('newname/tests/volatile/newname/')))

        out = StringIO()
        sys.stout = out

        call_command('makemigrations', 'newname', stdout=out)

        self.migrations_after = list(filter(lambda f: f.endswith('.py') and f != '__init__.py', os.listdir('newname/tests/volatile/newname/')))

        assert_that(out.getvalue(), equal_to("No changes detected in app 'newname'\n"))
        assert_that(self.migrations_after, has_length(len(self.migrations_before)))

    @override_settings(MIGRATION_MODULES={"tests": "newname.tests.volatile.newname_tests"})
    def test__shouldNotKeepRecreatingMigrationsWhenNoChange(self):
        call_command('makemigrations', 'tests')

        self.migrations_before = list(filter(lambda f: f.endswith('.py') and f != '__init__.py', os.listdir('newname/tests/volatile/newname_tests/')))
        ...
        self.migrations_after = list(filter(lambda f: f.endswith('.py') and f != '__init__.py', os.listdir('newname/tests/volatile/newname_tests/')))
    ...
    def test__shouldMigrateTransitionApprovalStatusToStringInDB(self):
        ...
        with connection.cursor() as cur:
            result = cur.execute("select status from newname_transitionapproval where object_id=%s;" % workflowmodel_object.model.pk).fetchall()
            assert_that(result[0][0], equal_to("pending"))

        call_command('migrate', 'newname', '0004', stdout=out)

        with connection.cursor() as cur:
            schema = cur.execute("PRAGMA table_info('newname_transitionapproval');").fetchall()
        ...
        call_command('migrate', 'newname', '0005', stdout=out)
    ...
    def test__shouldAssessIterationsForExistingApprovals(self):
        ...
        with connection.cursor() as cur:
            result = cur.execute("""
                select 
                    newname_transitionapproval.meta_id, 
                    iteration 
                from newname_transitionapproval 
                inner join newname_transition rt on newname_transitionapproval.transition_id = rt.id  
                where newname_transitionapproval.object_id=%s;
        ...
        call_command('migrate', 'newname', '0006', stdout=out)
        ...
        call_command('migrate', 'newname', '0007', stdout=out)

        with connection.cursor() as cur:
            result = cur.execute("select meta_id, iteration from newname_transitionapproval where object_id=%s;" % workflowmodel_object.model.pk).fetchall()
            assert_that(result, has_length(4))
            assert_that(result, has_item(equal_to((meta_1.pk, 0))))
            assert_that(result, has_item(equal_to((meta_2.pk, 1))))
            assert_that(result, has_item(equal_to((meta_3.pk, 1))))
    ...
    def test__shouldAssessIterationsForExistingApprovalsWhenThereIsCycle(self):
        ...
        workflowmodel_object.model.newname.my_field.approve(as_user=authorized_user)
        ...
        workflowmodel_object.model.newname.my_field.approve(as_user=authorized_user, next_state=cycle_state_1)
        assert_that(workflowmodel_object.model.my_field, equal_to(cycle_state_1))
    ...
    def test__shouldAssessIterationsForExistingApprovalsWhenThereIsMoreAdvanceCycle(self):
        ...
        workflowmodel_object.model.newname.my_field.approve(as_user=authorized_user, next_state=re_opened)
    ...
    def test__shouldCreateTransitionsAndTransitionMetasOutOfApprovalMetaAndApprovals(self):
        ...
        with connection.cursor() as cur:
            result = cur.execute("select id, transition_meta_id from newname_transitionapprovalmeta;").fetchall()
            assert_that(result, has_length(4))
            ...
            result = cur.execute("select id, transition_id from newname_transitionapproval where object_id='%s';" % workflowmodel_object.model.pk).fetchall()
            assert_that(result, has_length(4))
        ...
        call_command('migrate', 'newname', '0009', stdout=out)

        with connection.cursor() as cur:
            schema = cur.execute("PRAGMA table_info('newname_transitionapprovalmeta');").fetchall()
        ...
        call_command('migrate', 'newname', '0010', stdout=out)
    ...
    def test__shouldMigrateObjectIdInHooksByCastingItToString(self):
        ...
        call_command('migrate', 'newname', '0013', stdout=out)

        with connection.cursor() as cur:
            schema = cur.execute("PRAGMA table_info('newname_onapprovedhook');").fetchall()
            ...
            schema = cur.execute("PRAGMA table_info('newname_ontransithook');").fetchall()
            column_type = next(iter([column[2] for column in schema if column[1] == 'object_id']), None)
            assert_that(column_type, equal_to("integer unsigned"))

            schema = cur.execute("PRAGMA table_info('newname_oncompletehook');").fetchall()
            column_type = next(iter([column[2] for column in schema if column[1] == 'object_id']), None)
            assert_that(column_type, equal_to("integer unsigned"))

            result = cur.execute("select object_id from newname_onapprovedhook where object_id='%s';" % workflowmodel_object.model.pk).fetchall()
            assert_that(result, has_length(1))
            assert_that(result[0][0], equal_to(workflowmodel_object.model.pk))

            result = cur.execute("select object_id from newname_ontransithook where object_id='%s';" % workflowmodel_object.model.pk).fetchall()
            assert_that(result, has_length(1))
            assert_that(result[0][0], equal_to(workflowmodel_object.model.pk))

            result = cur.execute("select object_id from newname_oncompletehook where object_id='%s';" % workflowmodel_object.model.pk).fetchall()
            assert_that(result, has_length(1))
            assert_that(result[0][0], equal_to(workflowmodel_object.model.pk))

        call_command('migrate', 'newname', '0014', stdout=out)
