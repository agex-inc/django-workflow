...
from newname.models.state import State
from newname.models.transitionapproval import TransitionApproval
from newname.models.transition import Transition
...
class StateField(models.ForeignKey):
    ...
    def contribute_to_class(self, cls, name, *args, **kwargs):
        ...
        if id(cls) not in workflowmodel_registry.workflowmodels:
            self._add_to_class(cls, "newname", newname)

        super(StateField, self).contribute_to_class(cls, name, *args, **kwargs)

        if id(cls) not in workflowmodel_registry.workflowmodels:
            post_save.connect(_on_workflowmodel_object_saved, self.model, False, dispatch_uid='%s_%s_newnamestatefield_post' % (self.model, name))
            post_delete.connect(_on_workflowmodel_object_deleted, self.model, False, dispatch_uid='%s_%s_newnamestatefield_post' % (self.model, name))
...
def _on_workflowmodel_object_saved(sender, instance, created, *args, **kwargs):
    for instance_workflowmodel in instance.newname.all(instance.__class__):
        if created:
            instance_workflowmodel.initialize_approvals()
            if not instance_workflowmodel.get_state():
                init_state = getattr(instance.__class__.newname, instance_workflowmodel.field_name).initial_state
