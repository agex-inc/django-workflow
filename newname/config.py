    ...
    def settings(self):
        if self.cached_settings:
            ...
            for key, default in allowed_configurations.items():
                newname_settings[key] = getattr(settings, self.get_with_prefix(key), default)

            newname_settings['IS_MSSQL'] = connection.vendor == 'microsoft'
            self.cached_settings = newname_settings
