...
from newname.models import TransitionApproval, PENDING, State, APPROVED, WorkflowModel, CANCELLED, Transition, DONE, JUMPED
from newname.signals import ApproveSignal, TransitionSignal, OnCompleteSignal
from newname.utils.error_code import ErrorCode
...
class InstanceWorkflowModelObject(object):
    ...
    def __init__(self, workflowmodel_object, field_name):
        self.class_workflowmodel = getattr(workflowmodel_object.__class__.newname, field_name)
    ...
    def jump_to(self, state):
        ...
        try:
            ...
            raise NewNameException(ErrorCode.STATE_IS_NOT_AVAILABLE_TO_BE_JUMPED, "This state is not available to be jumped in the future of this object")
    ...
    def get_available_approvals(self, as_user=None, destination_state=None):
        qs = self.class_workflowmodel.get_available_approvals(as_user, ).filter(object_id=self.workflowmodel_object.pk)
    ...
    def approve(self, as_user, next_state=None):
        ...
        if number_of_available_approvals == 0:
            raise NewNameException(ErrorCode.NO_AVAILABLE_NEXT_STATE_FOR_USER, "There is no available approval for the user.")
        elif next_state:
            ...
            if available_approvals.count() == 0:
                ...
                raise NewNameException(ErrorCode.INVALID_NEXT_STATE_FOR_USER, "Invalid state is given(%s). Valid states is(are) %s" % (
                    next_state.__str__(), ','.join([ast.__str__() for ast in available_states])))
        elif number_of_available_approvals > 1 and not next_state:
            raise NewNameException(ErrorCode.NEXT_STATE_IS_REQUIRED, "State must be given when there are multiple states for destination")
