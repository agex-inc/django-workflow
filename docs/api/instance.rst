...
like in the example below;
    ...
    .. code-block:: python
        ...
        my_model.newname.my_state_field.<function>(*args)


approve
-------

This is the function that helps you to approve next approval of the object easily. ``django-newname`` will handle all
the availability and the authorization issues.

>>> my_model.newname.my_state_field.approve(as_user=team_leader)
>>> my_model.newname.my_state_field.approve(as_user=team_leader, next_state=State.objects.get(name='re_opened_state'))

+------------+-------+---------+------------+-------------+-----------------------------------------+
|            | Type  | Default |  Optional  |   Format    |               Description               |
+============+=======+=========+============+=============+=========================================+
| as_user    | input | NaN     | False      | Django User | | A user to make the transaction.       |
|            |       |         |            |             | | ``django-newname`` will check           |
...
destination states. If the source state is not provided, ``django-newname`` will pick the current objects source state.

>>> transition_approvals = my_model.newname.my_state_field.get_available_approvals(as_user=manager)
>>> transition_approvals = my_model.newname.my_state_field.get_available_approvals(as_user=manager, source_state=State.objects.get(name='in_progress'))
>>> transition_approvals = my_model.newname.my_state_field.get_available_approvals(
        as_user=manager, 
...
>>> transition_approval = my_model.newname.my_state_field.last_approval
...
>>> transition_approvals == my_model.newname.my_state_field.next_approvals
True
...
>>> my_model.newname.my_state_field.on_initial_state
True
...
>>> my_model.newname.my_state_field.on_final_state
True
...
>>> transition_approvals = my_model.newname.my_state_field.jump_to(in_progress_state)
