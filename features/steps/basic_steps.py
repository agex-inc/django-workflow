...
def permission(context, name):
    from workflow_config.models.factories import PermissionObjectFactory
...
def group(context, name):
    from workflow_config.models.factories import GroupObjectFactory
...
def user_with_permission(context, name, permission_name):
    ...
    from workflow_config.models.factories import UserObjectFactory
...
def state_with_label(context, state_label):
    from workflow_config.models.factories import StateObjectFactory
...
def transition(context, source_state_label, destination_state_label, workflow_identifier):
    ...
    from workflow_config.models.factories import TransitionMetaFactory
    from workflow_config.tests.models import BasicTestModel
    from workflow_config.models.factories import WorkflowFactory
...
def authorization_rule_with_permission(context, source_state_label, destination_state_label, permission_name, priority):
    from django.contrib.auth.models import Permission
    from workflow_config.models.factories import TransitionApprovalMetaFactory
...
def jump_workflow_object(context, workflow_object_identifier, state_label):
    ...
    workflow_object.workflow_config.my_field.jump_to(state)
...
def fetched_approvals(context, username):
    ...
    context.result = BasicTestModel.workflow_config.my_field.get_on_approval_objects(as_user=user)
...
def approve_by(context, workflow_object_identifier, username):
    ...
    user = User.objects.get(username=username)
    workflow_object.workflow_config.my_field.approve(as_user=user)
...
def approve_for_next_state_by(context, workflow_object_identifier, next_state, username):
    ...
    user = User.objects.get(username=username)
    workflow_object.workflow_config.my_field.approve(as_user=user, next_state=State.objects.get(label=next_state))
