...
from workflow_config.models import TransitionApproval, PENDING, State, APPROVED, Workflow, CANCELLED, Transition, DONE, JUMPED
from workflow_config.signals import ApproveSignal, TransitionSignal, OnCompleteSignal
from workflow_config.utils.error_code import ErrorCode
...
class InstanceWorkflowObject(object):
    ...
    def __init__(self, workflow_object, field_name):
        self.class_workflow = getattr(workflow_object.__class__.workflow_config, field_name)
    ...
    def jump_to(self, state):
        ...
        try:
            ...
            raise WorkflowConfigException(ErrorCode.STATE_IS_NOT_AVAILABLE_TO_BE_JUMPED, "This state is not available to be jumped in the future of this object")
    ...
    def approve(self, as_user, next_state=None):
        ...
        if number_of_available_approvals == 0:
            raise WorkflowConfigException(ErrorCode.NO_AVAILABLE_NEXT_STATE_FOR_USER, "There is no available approval for the user.")
        elif next_state:
            ...
            if available_approvals.count() == 0:
                ...
                raise WorkflowConfigException(ErrorCode.INVALID_NEXT_STATE_FOR_USER, "Invalid state is given(%s). Valid states is(are) %s" % (
                    next_state.__str__(), ','.join([ast.__str__() for ast in available_states])))
        elif number_of_available_approvals > 1 and not next_state:
            raise WorkflowConfigException(ErrorCode.NEXT_STATE_IS_REQUIRED, "State must be given when there are multiple states for destination")
