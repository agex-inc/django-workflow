...
# from workflow_config.driver.workflow_config.mssql_workflow_config import MsSqlworkflow_config
from workflow_config.driver.mssql_driver import MsSqlworkflow_config
from workflow_config.driver.orm_driver import Ormworkflow_config
from workflow_config.models import State, TransitionApprovalMeta, Workflow, app_config, TransitionMeta
...
class ClassWorkflowObject(object):
    ...
    def __init__(self, wokflow_object_class, field_name):
        ...
        self._cached_workflow_config_workflow_config = None

    @property
    def _workflow_config_workflow_config(self):
        if self._cached_workflow_config_workflow_config:
            return self._cached_workflow_config_workflow_config
        else:
            if app_config.IS_MSSQL:
                self._cached_workflow_config_workflow_config = MsSqlworkflow_config(self.workflow, self.wokflow_object_class, self.field_name)
            else:
                self._cached_workflow_config_workflow_config = Ormworkflow_config(self.workflow, self.wokflow_object_class, self.field_name)
            return self._cached_workflow_config_workflow_config
    ...
    def get_available_approvals(self, as_user):
        return self._workflow_config_workflow_config.get_available_approvals(as_user)
