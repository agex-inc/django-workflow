...
from workflow_config.models import TransitionApproval, OnApprovedHook, Function, OnCompleteHook, OnTransitHook
from workflow_config.models.factories import StateObjectFactory, WorkflowFactory, TransitionApprovalMetaFactory, PermissionObjectFactory, UserObjectFactory, \
...
def clean_migrations():
    for f in os.listdir("workflow_config/tests/volatile/workflow_config/"):
        if f != "__init__.py" and f != "__pycache__":
            os.remove(os.path.join("workflow_config/tests/volatile/workflow_config/", f))

    for f in os.listdir("workflow_config/tests/volatile/workflow_config_tests/"):
        if f != "__init__.py" and f != "__pycache__":
            os.remove(os.path.join("workflow_config/tests/volatile/workflow_config_tests/", f))


class MigrationTests(TestCase):
    """
    This is the case to detect missing migration issues like https://github.com/javrasya/django-workflow_config/issues/30
    """
    ...
    @override_settings(MIGRATION_MODULES={"workflow_config": "workflow_config.tests.volatile.workflow_config"})
    def test_shouldCreateAllMigrations(self):
        for f in os.listdir("workflow_config/migrations"):
            if f != "__init__.py" and f != "__pycache__" and not f.endswith(".pyc"):
                open(os.path.join("workflow_config/tests/volatile/workflow_config/", f), 'wb').write(open(os.path.join("workflow_config/migrations", f), 'rb').read())

        self.migrations_before = list(filter(lambda f: f.endswith('.py') and f != '__init__.py', os.listdir('workflow_config/tests/volatile/workflow_config/')))

        out = StringIO()
        sys.stout = out

        call_command('makemigrations', 'workflow_config', stdout=out)

        self.migrations_after = list(filter(lambda f: f.endswith('.py') and f != '__init__.py', os.listdir('workflow_config/tests/volatile/workflow_config/')))

        assert_that(out.getvalue(), equal_to("No changes detected in app 'workflow_config'\n"))
        assert_that(self.migrations_after, has_length(len(self.migrations_before)))

    @override_settings(MIGRATION_MODULES={"tests": "workflow_config.tests.volatile.workflow_config_tests"})
    def test__shouldNotKeepRecreatingMigrationsWhenNoChange(self):
        call_command('makemigrations', 'tests')

        self.migrations_before = list(filter(lambda f: f.endswith('.py') and f != '__init__.py', os.listdir('workflow_config/tests/volatile/workflow_config_tests/')))
        ...
        self.migrations_after = list(filter(lambda f: f.endswith('.py') and f != '__init__.py', os.listdir('workflow_config/tests/volatile/workflow_config_tests/')))
    ...
    def test__shouldMigrateTransitionApprovalStatusToStringInDB(self):
        ...
        with connection.cursor() as cur:
            result = cur.execute("select status from workflow_config_transitionapproval where object_id=%s;" % workflow_object.model.pk).fetchall()
            assert_that(result[0][0], equal_to("pending"))

        call_command('migrate', 'workflow_config', '0004', stdout=out)

        with connection.cursor() as cur:
            schema = cur.execute("PRAGMA table_info('workflow_config_transitionapproval');").fetchall()
        ...
        call_command('migrate', 'workflow_config', '0005', stdout=out)
    ...
    def test__shouldAssessIterationsForExistingApprovals(self):
        ...
        with connection.cursor() as cur:
            result = cur.execute("""
                select 
                    workflow_config_transitionapproval.meta_id, 
                    iteration 
                from workflow_config_transitionapproval 
                inner join workflow_config_transition rt on workflow_config_transitionapproval.transition_id = rt.id  
                where workflow_config_transitionapproval.object_id=%s;
        ...
        call_command('migrate', 'workflow_config', '0006', stdout=out)
        ...
        call_command('migrate', 'workflow_config', '0007', stdout=out)

        with connection.cursor() as cur:
            result = cur.execute("select meta_id, iteration from workflow_config_transitionapproval where object_id=%s;" % workflow_object.model.pk).fetchall()
            assert_that(result, has_length(4))
            assert_that(result, has_item(equal_to((meta_1.pk, 0))))
            assert_that(result, has_item(equal_to((meta_2.pk, 1))))
            assert_that(result, has_item(equal_to((meta_3.pk, 1))))
    ...
    def test__shouldAssessIterationsForExistingApprovalsWhenThereIsCycle(self):
        ...
        workflow_object.model.workflow_config.my_field.approve(as_user=authorized_user)
        ...
        workflow_object.model.workflow_config.my_field.approve(as_user=authorized_user, next_state=cycle_state_1)
        assert_that(workflow_object.model.my_field, equal_to(cycle_state_1))
    ...
    def test__shouldAssessIterationsForExistingApprovalsWhenThereIsMoreAdvanceCycle(self):
        ...
        workflow_object.model.workflow_config.my_field.approve(as_user=authorized_user, next_state=re_opened)
    ...
    def test__shouldCreateTransitionsAndTransitionMetasOutOfApprovalMetaAndApprovals(self):
        ...
        with connection.cursor() as cur:
            result = cur.execute("select id, transition_meta_id from workflow_config_transitionapprovalmeta;").fetchall()
            assert_that(result, has_length(4))
            ...
            result = cur.execute("select id, transition_id from workflow_config_transitionapproval where object_id='%s';" % workflow_object.model.pk).fetchall()
            assert_that(result, has_length(4))
        ...
        call_command('migrate', 'workflow_config', '0009', stdout=out)

        with connection.cursor() as cur:
            schema = cur.execute("PRAGMA table_info('workflow_config_transitionapprovalmeta');").fetchall()
        ...
        call_command('migrate', 'workflow_config', '0010', stdout=out)
    ...
    def test__shouldMigrateObjectIdInHooksByCastingItToString(self):
        ...
        call_command('migrate', 'workflow_config', '0013', stdout=out)

        with connection.cursor() as cur:
            schema = cur.execute("PRAGMA table_info('workflow_config_onapprovedhook');").fetchall()
            ...
            schema = cur.execute("PRAGMA table_info('workflow_config_ontransithook');").fetchall()
            column_type = next(iter([column[2] for column in schema if column[1] == 'object_id']), None)
            assert_that(column_type, equal_to("integer unsigned"))

            schema = cur.execute("PRAGMA table_info('workflow_config_oncompletehook');").fetchall()
            column_type = next(iter([column[2] for column in schema if column[1] == 'object_id']), None)
            assert_that(column_type, equal_to("integer unsigned"))

            result = cur.execute("select object_id from workflow_config_onapprovedhook where object_id='%s';" % workflow_object.model.pk).fetchall()
            assert_that(result, has_length(1))
            assert_that(result[0][0], equal_to(workflow_object.model.pk))

            result = cur.execute("select object_id from workflow_config_ontransithook where object_id='%s';" % workflow_object.model.pk).fetchall()
            assert_that(result, has_length(1))
            assert_that(result[0][0], equal_to(workflow_object.model.pk))

            result = cur.execute("select object_id from workflow_config_oncompletehook where object_id='%s';" % workflow_object.model.pk).fetchall()
            assert_that(result, has_length(1))
            assert_that(result[0][0], equal_to(workflow_object.model.pk))

        call_command('migrate', 'workflow_config', '0014', stdout=out)
