...
from workflow_config.models import TransitionApproval, PENDING, CANCELLED, APPROVED, Transition, JUMPED
from workflow_config.models.factories import UserObjectFactory, PermissionObjectFactory
from workflow_config.tests.matchers import has_approval
from workflow_config.tests.models import BasicTestModel, ModelWithTwoStateFields, ModelWithStringPrimaryKey
from workflow_config.tests.models.factories import ModelWithTwoStateFieldsObjectFactory
from workflow_config.utils.exceptions import WorkflowConfigException
# noinspection PyMethodMayBeStatic,DuplicatedCode
from workflow_configtest.flowbuilder import FlowBuilder, AuthorizationPolicyBuilder, RawState
...
class InstanceApiTest(TestCase):
    ...
    def test_shouldNotReturnOtherObjectsApprovalsForTheAuthorizedUser(self):
        ...
        available_approvals = workflow_object1.workflow_config.my_field.get_available_approvals(as_user=authorized_user)
    ...
    def test_shouldNotAllowUnauthorizedUserToProceedToNextState(self):
        ...
        assert_that(
            calling(workflow_object.workflow_config.my_field.approve).with_args(as_user=unauthorized_user),
            raises(WorkflowConfigException, "There is no available approval for the user")
        )
    ...
    def test_shouldNotLetUserWhosePriorityComesLaterApproveProceed(self):
        ...
        assert_that(
            calling(workflow_object.workflow_config.my_field.approve).with_args(as_user=manager),
            raises(WorkflowConfigException, "There is no available approval for the user")
        )
    ...
    def test_shouldNotTransitToNextStateWhenThereAreMultipleApprovalsToBeApproved(self):
        ...
        assert_that(workflow_object.my_field, equal_to(flow.get_state(state1)))
        workflow_object.workflow_config.my_field.approve(team_leader)
        assert_that(workflow_object.my_field, equal_to(flow.get_state(state1)))
    ...
    def test_shouldTransitToNextStateWhenAllTheApprovalsAreApproved(self):
        ...
        assert_that(workflow_object.my_field, equal_to(flow.get_state(state1)))
        workflow_object.workflow_config.my_field.approve(manager)
        assert_that(workflow_object.my_field, equal_to(flow.get_state(state2)))
    ...
    def test_shouldDictatePassingNextStateWhenThereAreMultiple(self):
        ...
        workflow_object = flow.objects[0]
        assert_that(
            calling(workflow_object.workflow_config.my_field.approve).with_args(as_user=authorized_user),
            raises(WorkflowConfigException, "State must be given when there are multiple states for destination")
        )
    ...
    def test_shouldTransitToTheGivenNextStateWhenThereAreMultipleNextStates(self):
        ...
        assert_that(workflow_object.my_field, equal_to(flow.get_state(state1)))
        workflow_object.workflow_config.my_field.approve(as_user=authorized_user, next_state=flow.get_state(state3))
    ...
    def test_shouldNotAcceptANextStateWhichIsNotAmongPossibleNextStates(self):
        ...
        assert_that(
            calling(workflow_object.workflow_config.my_field.approve).with_args(as_user=authorized_user, next_state=flow.get_state(invalid_state)),
            raises(WorkflowConfigException,
    ...
    def test_shouldAllowCyclicTransitions(self):
        ...
        workflow_object.workflow_config.my_field.approve(as_user=authorized_user, next_state=flow.get_state(cycle_state_1))
        assert_that(workflow_object.my_field, equal_to(flow.get_state(cycle_state_1)))
    ...
    def test__shouldReturnNextApprovals(self):
        ...
        assert_that(workflow_object.my_field, equal_to(flow.get_state(state1)))
        next_approvals = workflow_object.workflow_config.my_field.next_approvals
    ...
    def test_shouldNotCancelDescendantsIfItIsPartOfPossibleFuture(self):
        ...
        workflow_object.workflow_config.my_field.approve(as_user=authorized_user, next_state=flow.get_state(diamond_left_state_1))
    ...
    def test_shouldJumpToASpecificState(self):
        ...
        workflow_object.workflow_config.my_field.jump_to(flow.get_state(state3))
    ...
    def test_shouldNotJumpBackToAPreviousState(self):
        ...
        workflow_object.workflow_config.my_field.approve(as_user=authorized_user, next_state=flow.get_state(state2))
        assert_that(
            calling(workflow_object.workflow_config.my_field.jump_to).with_args(flow.get_state(state1)),
            raises(WorkflowConfigException, "This state is not available to be jumped in the future of this object")
        )
    ...
    def test_shouldNotCrashWhenAModelObjectWithStringPrimaryKeyIsApproved(self):
        ...
        workflow_object.workflow_config.status.approve(as_user=authorized_user)
    ...
    def test_shouldAllowMultipleCyclicTransitions(self):
        ...
        workflow_object.workflow_config.my_field.approve(as_user=authorized_user, next_state=flow.get_state(cycle_state_2))
        assert_that(workflow_object.my_field, equal_to(flow.get_state(cycle_state_2)))
