...
from workflow_config.models.factories import PermissionObjectFactory, UserObjectFactory, StateObjectFactory, GroupObjectFactory
...
class ClassApiTest(TestCase):
    ...
    def test_shouldReturnNoApprovalWhenUserIsUnAuthorized(self):
        ...
        available_approvals = BasicTestModel.workflow_config.my_field.get_available_approvals(as_user=unauthorized_user)
    ...
    def test_shouldReturnAnApprovalWhenUserIsAuthorizedWithAPermission(self):
        ...
        available_approvals = BasicTestModel.workflow_config.my_field.get_available_approvals(as_user=authorized_user)
    ...
    def test__shouldReturnApprovalsOnTimeWhenTooManyWorkflowObject(self):
        ...
        before = datetime.now()
        approvals = BasicTestModel.workflow_config.my_field.get_on_approval_objects(as_user=authorized_user)
        after = datetime.now()
    ...
    def test_shouldAssesInitialStateProperly(self):
        ...
        assert_that(BasicTestModel.workflow_config.my_field.initial_state, equal_to(flow.get_state(state1)))
    ...
    def test_shouldAssesFinalStateProperlyWhenThereIsOnlyOne(self):
        ...
        assert_that(BasicTestModel.workflow_config.my_field.final_states, has_length(1))
        assert_that(list(BasicTestModel.workflow_config.my_field.final_states), has_item(flow.get_state(state2)))
    ...
    def test_shouldAssesFinalStateProperlyWhenThereAreMultiple(self):
        ...
        assert_that(BasicTestModel.workflow_config.my_field.final_states, has_length(4))
        assert_that(
            list(BasicTestModel.workflow_config.my_field.final_states),
            has_items(
