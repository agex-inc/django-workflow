...
from workflow_config.driver.workflow_config_driver import WorkflowConfigworkflow_config
from workflow_config.models import TransitionApproval


class MsSqlworkflow_config(WorkflowConfigworkflow_config):

    def __init__(self, *args, **kwargs):
        super(MsSqlworkflow_config, self).__init__(*args, **kwargs)
        with open(os.path.join(dirname(dirname(__file__)), "sql", "mssql", "get_available_approvals.sql")) as f:
            self.available_approvals_sql_template = f.read().replace("workflow_config.dbo.", "")
