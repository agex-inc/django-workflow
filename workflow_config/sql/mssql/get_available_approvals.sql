WITH approvals_with_min_priority (workflow_id, transition_id, object_id, min_priority) AS
         (
             ...
             FROM workflow_config.dbo.workflow_config_transitionapproval
     ...
     authorized_approvals(id, workflow_id, transition_id, source_state_id, object_id, priority) AS
         (
             SELECT ta.id,
                    ...
                    ta.object_id,
                    ta.priority
             FROM workflow_config.dbo.workflow_config_transitionapproval ta
                      INNER JOIN workflow_config.dbo.workflow_config_transition t on t.id = ta.transition_id
                      LEFT JOIN workflow_config.dbo.workflow_config_transitionapproval_permissions tap on tap.transitionapproval_id = ta.id
                      LEFT JOIN workflow_config.dbo.workflow_config_transitionapproval_groups tag on tag.transitionapproval_id = ta.id
