...
from workflow_config.models import OnApprovedHook, OnTransitHook, OnCompleteHook
...
from workflow_config.models.state import State
from workflow_config.models.transitionapproval import TransitionApproval
from workflow_config.models.transition import Transition
...
class StateField(models.ForeignKey):
    ...
    def contribute_to_class(self, cls, name, *args, **kwargs):
        @classproperty
        def workflow_config(_self):
            return WorkflowConfigObject(_self)
        ...
        if id(cls) not in workflow_registry.workflows:
            self._add_to_class(cls, "workflow_config", workflow_config)

        super(StateField, self).contribute_to_class(cls, name, *args, **kwargs)

        if id(cls) not in workflow_registry.workflows:
            post_save.connect(_on_workflow_object_saved, self.model, False, dispatch_uid='%s_%s_workflow_configstatefield_post' % (self.model, name))
            post_delete.connect(_on_workflow_object_deleted, self.model, False, dispatch_uid='%s_%s_workflow_configstatefield_post' % (self.model, name))
...
def _on_workflow_object_saved(sender, instance, created, *args, **kwargs):
    for instance_workflow in instance.workflow_config.all(instance.__class__):
        if created:
            instance_workflow.initialize_approvals()
            if not instance_workflow.get_state():
                init_state = getattr(instance.__class__.workflow_config, instance_workflow.field_name).initial_state
