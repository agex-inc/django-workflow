...
from workflow.models import TransitionApproval, OnApprovedHook, Function, OnCompleteHook, OnTransitHook
from workflow.models.factories import StateObjectFactory, WorkflowModelFactory, TransitionApprovalMetaFactory, PermissionObjectFactory, UserObjectFactory, \
...
def clean_migrations():
    for f in os.listdir("workflow/tests/volatile/workflow/"):
        if f != "__init__.py" and f != "__pycache__":
            os.remove(os.path.join("workflow/tests/volatile/workflow/", f))

    for f in os.listdir("workflow/tests/volatile/workflow_tests/"):
        if f != "__init__.py" and f != "__pycache__":
            os.remove(os.path.join("workflow/tests/volatile/workflow_tests/", f))


class MigrationTests(TestCase):
    """
    This is the case to detect missing migration issues like https://github.com/javrasya/django-workflow/issues/30
    """
    ...
    @override_settings(MIGRATION_MODULES={"workflow": "workflow.tests.volatile.workflow"})
    def test_shouldCreateAllMigrations(self):
        for f in os.listdir("workflow/migrations"):
            if f != "__init__.py" and f != "__pycache__" and not f.endswith(".pyc"):
                open(os.path.join("workflow/tests/volatile/workflow/", f), 'wb').write(open(os.path.join("workflow/migrations", f), 'rb').read())

        self.migrations_before = list(filter(lambda f: f.endswith('.py') and f != '__init__.py', os.listdir('workflow/tests/volatile/workflow/')))

        out = StringIO()
        sys.stout = out

        call_command('makemigrations', 'workflow', stdout=out)

        self.migrations_after = list(filter(lambda f: f.endswith('.py') and f != '__init__.py', os.listdir('workflow/tests/volatile/workflow/')))

        assert_that(out.getvalue(), equal_to("No changes detected in app 'workflow'\n"))
        assert_that(self.migrations_after, has_length(len(self.migrations_before)))

    @override_settings(MIGRATION_MODULES={"tests": "workflow.tests.volatile.workflow_tests"})
    def test__shouldNotKeepRecreatingMigrationsWhenNoChange(self):
        call_command('makemigrations', 'tests')

        self.migrations_before = list(filter(lambda f: f.endswith('.py') and f != '__init__.py', os.listdir('workflow/tests/volatile/workflow_tests/')))
        ...
        self.migrations_after = list(filter(lambda f: f.endswith('.py') and f != '__init__.py', os.listdir('workflow/tests/volatile/workflow_tests/')))
    ...
    def test__shouldMigrateTransitionApprovalStatusToStringInDB(self):
        ...
        workflowmodel = WorkflowModelFactory(initial_state=state1, content_type=ContentType.objects.get_for_model(BasicTestModel), field_name="my_field")
        ...
        workflowmodel_object = BasicTestModelObjectFactory()

        with connection.cursor() as cur:
            result = cur.execute("select status from workflow_transitionapproval where object_id=%s;" % workflowmodel_object.model.pk).fetchall()
            assert_that(result[0][0], equal_to("pending"))

        call_command('migrate', 'workflow', '0004', stdout=out)

        with connection.cursor() as cur:
            schema = cur.execute("PRAGMA table_info('workflow_transitionapproval');").fetchall()
        ...
        call_command('migrate', 'workflow', '0005', stdout=out)
    ...
    def test__shouldAssessIterationsForExistingApprovals(self):
        ...
        with connection.cursor() as cur:
            result = cur.execute("""
                select 
                    workflow_transitionapproval.meta_id, 
                    iteration 
                from workflow_transitionapproval 
                inner join workflow_transition rt on workflow_transitionapproval.transition_id = rt.id  
                where workflow_transitionapproval.object_id=%s;
             """ % workflowmodel_object.model.pk).fetchall()
        ...
        call_command('migrate', 'workflow', '0006', stdout=out)
        ...
        call_command('migrate', 'workflow', '0007', stdout=out)

        with connection.cursor() as cur:
            result = cur.execute("select meta_id, iteration from workflow_transitionapproval where object_id=%s;" % workflowmodel_object.model.pk).fetchall()
            assert_that(result, has_length(4))
            assert_that(result, has_item(equal_to((meta_1.pk, 0))))
            assert_that(result, has_item(equal_to((meta_2.pk, 1))))
            assert_that(result, has_item(equal_to((meta_3.pk, 1))))
    ...
    def test__shouldAssessIterationsForExistingApprovalsWhenThereIsCycle(self):
        ...
        workflowmodel = WorkflowModelFactory(initial_state=cycle_state_1, content_type=ContentType.objects.get_for_model(BasicTestModel), field_name="my_field")
        ...
        assert_that(workflowmodel_object.model.my_field, equal_to(cycle_state_1))
        workflowmodel_object.model.workflow.my_field.approve(as_user=authorized_user)
        assert_that(workflowmodel_object.model.my_field, equal_to(cycle_state_2))
        workflowmodel_object.model.workflow.my_field.approve(as_user=authorized_user)
        assert_that(workflowmodel_object.model.my_field, equal_to(cycle_state_2))
        workflowmodel_object.model.workflow.my_field.approve(as_user=authorized_user)
        assert_that(workflowmodel_object.model.my_field, equal_to(cycle_state_3))

        approvals = TransitionApproval.objects.filter(workflowmodel=workflowmodel, workflowmodel_object=workflowmodel_object.model)
        assert_that(approvals, has_length(5))

        workflowmodel_object.model.workflow.my_field.approve(as_user=authorized_user, next_state=cycle_state_1)
        assert_that(workflowmodel_object.model.my_field, equal_to(cycle_state_1))
    ...
    def test__shouldAssessIterationsForExistingApprovalsWhenThereIsMoreAdvanceCycle(self):
        ...
        workflowmodel = WorkflowModelFactory(initial_state=opn, content_type=ContentType.objects.get_for_model(BasicTestModel), field_name="my_field")
        ...
        assert_that(workflowmodel_object.model.my_field, equal_to(opn))
        workflowmodel_object.model.workflow.my_field.approve(as_user=authorized_user)
        assert_that(workflowmodel_object.model.my_field, equal_to(in_progress))
        workflowmodel_object.model.workflow.my_field.approve(as_user=authorized_user)
        assert_that(workflowmodel_object.model.my_field, equal_to(resolved))
        workflowmodel_object.model.workflow.my_field.approve(as_user=authorized_user, next_state=re_opened)
        assert_that(workflowmodel_object.model.my_field, equal_to(re_opened))
        workflowmodel_object.model.workflow.my_field.approve(as_user=authorized_user)
        assert_that(workflowmodel_object.model.my_field, equal_to(in_progress))
    ...
    def test__shouldCreateTransitionsAndTransitionMetasOutOfApprovalMetaAndApprovals(self):
        ...
        workflowmodel = WorkflowModelFactory(initial_state=state1, content_type=ContentType.objects.get_for_model(BasicTestModel), field_name="my_field")
        transition_1 = TransitionMetaFactory.create(workflowmodel=workflowmodel, source_state=state1, destination_state=state2)
        transition_2 = TransitionMetaFactory.create(workflowmodel=workflowmodel, source_state=state2, destination_state=state3)
        transition_3 = TransitionMetaFactory.create(workflowmodel=workflowmodel, source_state=state2, destination_state=state4)
        ...
        with connection.cursor() as cur:
            result = cur.execute("select id, transition_meta_id from workflow_transitionapprovalmeta;").fetchall()
            assert_that(result, has_length(4))
            ...
            result = cur.execute("select id, transition_id from workflow_transitionapproval where object_id='%s';" % workflowmodel_object.model.pk).fetchall()
            assert_that(result, has_length(4))
        ...
        call_command('migrate', 'workflow', '0009', stdout=out)

        with connection.cursor() as cur:
            schema = cur.execute("PRAGMA table_info('workflow_transitionapprovalmeta');").fetchall()
        ...
        call_command('migrate', 'workflow', '0010', stdout=out)
    ...
    def test__shouldMigrateObjectIdInHooksByCastingItToString(self):
        ...
        OnApprovedHook.objects.create(
            ...
            hook_type=BEFORE,
            workflowmodel_object=workflowmodel_object.model
        )
        ...
        call_command('migrate', 'workflow', '0013', stdout=out)

        with connection.cursor() as cur:
            schema = cur.execute("PRAGMA table_info('workflow_onapprovedhook');").fetchall()
            ...
            schema = cur.execute("PRAGMA table_info('workflow_ontransithook');").fetchall()
            column_type = next(iter([column[2] for column in schema if column[1] == 'object_id']), None)
            assert_that(column_type, equal_to("integer unsigned"))

            schema = cur.execute("PRAGMA table_info('workflow_oncompletehook');").fetchall()
            column_type = next(iter([column[2] for column in schema if column[1] == 'object_id']), None)
            assert_that(column_type, equal_to("integer unsigned"))

            result = cur.execute("select object_id from workflow_onapprovedhook where object_id='%s';" % workflowmodel_object.model.pk).fetchall()
            assert_that(result, has_length(1))
            assert_that(result[0][0], equal_to(workflowmodel_object.model.pk))

            result = cur.execute("select object_id from workflow_ontransithook where object_id='%s';" % workflowmodel_object.model.pk).fetchall()
            assert_that(result, has_length(1))
            assert_that(result[0][0], equal_to(workflowmodel_object.model.pk))

            result = cur.execute("select object_id from workflow_oncompletehook where object_id='%s';" % workflowmodel_object.model.pk).fetchall()
            assert_that(result, has_length(1))
            assert_that(result[0][0], equal_to(workflowmodel_object.model.pk))

        call_command('migrate', 'workflow', '0014', stdout=out)
        ...
        with connection.cursor() as cur:
            ...
            result = cur.execute("select object_id from workflow_onapprovedhook where object_id='%s';" % workflowmodel_object.model.pk).fetchall()
            assert_that(result, has_length(1))
            assert_that(result[0][0], equal_to(str(workflowmodel_object.model.pk)))
